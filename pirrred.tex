...
X^{p-3} + \cdots + p.
\]
Since $p$ divides ${p \choose i}$ for all $i$, the polynomial is
...
The following proposition, due to {\Ore} \cite{Ore23}, makes this
observation precise.
...
polynomials was suggested by {\Weinberger} \cite{Weinberger84}.  The
...
K =\Q[\alpha] \arrow{l}\arrow{d}   & A =\Z[\alpha]\arrow{d}\\
\Q\arrow{l} & \Z
\end{tikzcd}
\]
...
The following proposition, due to {\CohenS} \cite{CohenS81}, is the
...
Geometrically, Bertini's theorem \cite{Bertini82} says that the
...
\cite{Jouanolou83,Shafarevich:Algebraic:Geometry,Hartshorne:Algebraic:Geometry,Lang:Algebraic:Geometry}.
...
\[
d + {d \choose 2} + \cdots + {d \choose d-1} + 1 =2^d -1
\]
...
algorithms of this sort are discussed in \cite{Monagan92}.

{\Hilbert} \cite{Hilbert87} observed that 
\[
...
{\Szego} \cite{Polya:Szego}.   {\SwinnertonDyer} \cite{Berlekamp70}
...
\cite[Art.~341]{Gauss:Disquisitiones} by very different means. 

\notesectref{HIT:Sec} {\Runge} \cite{Runge86} used the Puiseux
...
$\Q$ have $\mathfrak{S}_n$ as their Galois group \cite{Waerden36}.  
...
\cite{Waerden36} and {\Knobloch} \cite{Knobloch55,Knobloch56} culminated 
with the following result of {\Gallagher} \cite{Gallagher73}:
\[
...
from Bertini's theorem in \cite{Gathen85b}.
