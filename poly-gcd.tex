...
two polynomial problem by the technique of \sectref{PRS:Content:Sec}.

The \keyi{cofactor} of $F$ is defined to be $F/H$, and the cofactor of
...
denote their \keyi{cofactors}\Marginpar{Need define this term
earlier}, \ie,
...
\longpropref{Factor:CBound:Prop} provides such a bound.  Since $H(X)$
...
\hat{G})$.  \longpropref{Resultant:Bound:Prop} gives a bound on the
size of $M$: 
\[
...
Initially, we will assume that $F$ and $G$ are monic.  In this case
...
This procedure is then used iteratively to recursively to produce
...
Second, instead of lifting the {\sc gcd} it is worthwhile to lift
